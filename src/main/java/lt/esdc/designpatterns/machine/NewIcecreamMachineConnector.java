...
public class NewIcecreamMachineConnector implements IcecreamMachineV77 {
    ...
    @Override
    public void makeDessert(String token, String session, String dessert) {
        validateToken(token);
        validateSession(session);
        validateOrder(dessert);

        System.out.println("Preparing dessert... " + dessert);
        System.out.println("..........");
        try {
            Thread.sleep(2000L);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("Unexpected interruption during dessert preparation", e);
        }
        ...
        if (!orderString.matches(ORDER_PATTERN)) {
            throw new IllegalArgumentException(
                    "Invalid order format: '" + orderString +
                            ". Expected format: <frozenMass>g <milk>ml <juice>ml <water>ml [toppings...]" +
                            "(e.g. '150g 200ml 0ml 20ml syrup')."
            );
        }
    }
}
